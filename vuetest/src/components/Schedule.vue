...
<template lang="html">
  <div class="schedule_box">
    ...
    <div :class="{'read_only':!setType}" :style="{'width':`${(tdSize*24)+dayWidth}px`,'min-height':`${tdSize*7 + timeHeight + flterHeight}px` }" >
      ...
      <div class="schedule_body_box">
        ...
        <div ref="time_bloack_box" class="time_bloack_box" @mousedown="down($event)" @mouseup="up($event)" @mousemove="move($event)">
          <div v-for="(itemf,indexf) in statue_data_array" :key="'f'+indexf" :style="{height: `${tdSize}px`}" >
            <span v-for="(item,index) in itemf"
                  :style="{'width':`${tdSize}px`,'height':`${tdSize}px`}"
                  :key=" 'c'+index + indexf "
                  :class="{'schedule_time_item_yes':item==1}">{{index}}
            </span>
            <i v-for="(item,index) in txt_statue_data[indexf]" :style="{'height':`${tdSize}px`,'line-height': `${tdSize}px`,'width':`${item.length*tdSize}px`,'left':`${item.start*tdSize}px`}">
              {{item.length==24 ? '全天投放' : `${item.start}:00 - ${item.end+1}:00`}}
            </i>
          </div>
          <em class="move_bg" :style="[move_bg_data]"></em>
        </div>
      </div>
      <div class="schedule_flter_box" :style="{ 'height': flterHeight+'px',' line-height':flterHeight+'px'}">
        <div class="flter_btn_box">
          快速设定：
          <span @click="dayTf('all')">全周投放</span>
...
export default {
    ...
    originVal:{
      ...
      validator(val) {
        if( !Array.isArray(val) ){
          console.log(new Error('originVal必须是Array'))
          return false;
        }
        let isY = val.every(item=>item===0||item===1);
        if(!isY || val.length !== 168){
          console.error(new Error('originVal必须是只包含1|0的168位数组'))
          return false
        }
        return true;
      }
  ...
  created(){
    //校验不通过功能打住
    if(this._validator_originVal_fn()) this.initSet_statue_data_array()
  },
  methods:{
    _validator_originVal_fn (val = this.originVal) {
      if( Array.isArray(val) ){
        let isY = val.every(item=>item===0||item===1);
        return (!isY || val.length !== 168) ? false : true;
      }
      return false;
    },

    /** */
    initSet_statue_data_array (oval = this.originVal) {
      let arry7 = []
      for(let i=0; i<7; i++){
        let a = [];
        for(let t=0; t<24; t++){ a[t]=(oval[i*24 + t])}
        arry7.push(a)
      }
      this.statue_data_array = arry7
    },
    /** 24小时快速选择 */
    timeTitle(index){
      if(!this.setType){ return}
      var narr = []
      var garr = this.statue_data_array.map(item=>item[index])
      this.statue_data_array.forEach((item,i)=>{
        if(garr.indexOf('0')>= 0 || garr.indexOf(0)>= 0){
          this.$set(this.statue_data_array[i],index,1 )
        }else{
          this.$set(this.statue_data_array[i],index,0 )
        }
      })
    },
    /** 星期1～7快速选择 */
    dayTitle(index){
      if(!this.setType){ return}
      if(this.statue_data_array[index].indexOf('0')>= 0 || this.statue_data_array[index].indexOf(0)>= 0){
        this.$set(this.statue_data_array,index,[...this.I].map(item=>item*1))
        return
      }
      this.$set(this.statue_data_array,index,[...this.C].map(item=>item*1))
    },
    /** 快速设置按钮方案： */
    dayTf(n){
      if(!this.setType){ return}
      switch (n) {
        case 'all':
          //全部投放
          this.statue_data_array.forEach((item,index)=>{
            this.$set(this.statue_data_array,index,[...this.I].map(item=>item*1))
          })
          break;
        case 'restDay':
    ...
    mousefn(){
      if(!this.setType){ return}
      /** 鼠标点击框下 处理函数*/
      var dayStart = (parseInt(this.move_bg_data.top) / this.tdSize); //天数组索引 start
      var dayEnd = parseInt(this.move_bg_data.height)/ this.tdSize + dayStart -1 ; //天数组索引 end
      var timeStart = (parseInt(this.move_bg_data.left)/ this.tdSize);//小时数组（子）索引 start
      var timeEnd = parseInt(this.move_bg_data.width)/ this.tdSize + timeStart-1;  //小时数组（子）索引 end
      for(var i = dayStart; i<=dayEnd; i++){
        let d = this.statue_data_array[i];
        for(var e = timeStart; e<=timeEnd; e++){
          this.$set(this.statue_data_array[i],e,d[e]==1 ? 0 : 1)
        }
      }
      this.move_bg_data = { display:'none' }
    }
  },
  computed:{
    txt_statue_data:function () {
      let txtShowArray = []
      this.statue_data_array.forEach(item=>{
  ...
  watch:{
    'statue_data_array':function (newQuestion, oldQuestion) {
      this.$emit('onchange',this.value)
    },
    'originVal':function(newData,oldData) {
       if(newData.toString()!==oldData.toString()){
         if(this._validator_originVal_fn()){
           this.initSet_statue_data_array()
         }else{
           console.error(new Error('“originVal” 必须是只包含1|0的168位数组'))
         }
       }
    }
  }
...
/* background:#fff; */
.schedule_box{padding: 1rem; background:#f5f5f5; display: inline-block; position: relative; overflow: hidden;  box-sizing:content-box;color: #1f2d3d;
...
.flter_msg_box{float: right; vertical-align: baseline; overflow: hidden;}
...
.flter_btn_box{ line-height:20px;}
.flter_btn_box>span{
