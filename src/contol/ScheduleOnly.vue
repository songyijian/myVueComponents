...
:disabled="true" #是否可编辑
:tdSize="30" #每个格子的大小(px) 不<16 :timeHeight="30" #0-23小时列快速选择按钮高度(px) 不<16 :dayWidth="50" #周1-7行速选择按钮宽度(px) 不<16
  [高度是根据tdSize算出来的] */ <template lang="html">
  <div class="schedule_box">
    <input type="hidden" :value="value">
    <div :class="{'disabledstyle':!disabled}"
      :style="{'width':`${(tdSize*24)+dayWidth}px`,'min-height':`${tdSize*7 + timeHeight + flterHeight}px` }">
      <div class="time_title_box" :style="{ 'margin-left': dayWidth+'px','height':timeHeight+'px'}">
        <span v-for="item,index in time" :key="'t'+index" :data-index="index" @click="timeTitle(index)"
          :style="{'width':`${tdSize}px`,'line-height':timeHeight+'px'}">{{item}}</span>
      </div>
      <div class="schedule_body_box">
        <div class="day_title_box" :style="{ 'width': dayWidth+'px' }">
          <span v-for="item,index in day" :key="item" @click="dayTitle(index)"
        ...
        <div ref="time_bloack_box" class="time_bloack_box" @mousedown="down($event)" @mouseup="up($event)"
          ...
          <div v-for="(itemf,indexf) in nowStatueArray" :key="'f'+indexf" :style="{height: `${tdSize}px`}">
            ...
            <i v-for="(item,index) in txtShowStatue[indexf]"
              :style="{'height':`${tdSize}px`,'line-height': `${tdSize}px`,'width':`${item.length*tdSize}px`,'left':`${item.start*tdSize}px`}">
