	...
	<div class="schedule_box">
		...
		<div
			...
			:style="{
				...
				'min-height': `${tdSize * 7 + timeHeight + flterHeight}px`
			}"
		>
			<div
				class="time_title_box"
				:style="{ 'margin-left': dayWidth + 'px', height: timeHeight + 'px' }"
			>
				<span
					v-for="(item, index) in time"
					:key="'t' + index"
					:data-index="index"
					@click="timeTitle(index)"
					:style="{ width: `${tdSize}px`, 'line-height': timeHeight + 'px' }"
				>
					{{ item }}
				</span>
			</div>
			<div class="schedule_body_box">
				<div class="day_title_box" :style="{ width: dayWidth + 'px' }">
					<span
						...
						:key="item"
						@click="dayTitle(index)"
						:style="{ height: `${tdSize}px`, 'line-height': `${tdSize}px` }"
					>
						{{ item }}
					</span>
				</div>
				...
				>
					<div
						v-for="(itemf, indexf) in nowStatueArray"
						:key="'f' + indexf"
						:style="{ height: `${tdSize}px` }"
					>
						<span
							v-for="(item, index) in itemf"
							:style="{ width: `${tdSize}px`, height: `${tdSize}px` }"
							:key="'c' + index + indexf"
							:class="{ schedule_time_item_yes: item == 1 }"
						>
							{{ index }}
						</span>
						<i
							v-for="(item, index) in txtShowStatue[indexf]"
							:style="{
								height: `${tdSize}px`,
...
export default {
	...
	methods: {
		...
		dayTitle(index) {
			...
			if (
				this.nowStatueArray[index].indexOf("0") >= 0 ||
				this.nowStatueArray[index].indexOf(0) >= 0
			) {
				...
				if (yl > 0) {
					if (y > this.downData.y + this.dowBgDval.bottom) {
						...
						if (y < this.downData.y + this.dowBgDval.bottom + this.tdSize) {
							...
							h =
								(Math.ceil((yl - this.dowBgDval.bottom) / this.tdSize) + 1) *
									this.tdSize +
								"px";
						}
					}
				...
				if (xl > 0) {
					if (x > this.downData.x + this.dowBgDval.right) {
						...
						if (x < this.downData.x + this.dowBgDval.right + this.tdSize) {
							...
							w =
								(Math.ceil((xl - this.dowBgDval.right) / this.tdSize) + 1) *
									this.tdSize +
								"px";
						}
					}
				...
				if (yl < 0) {
					if (y < this.downData.y - this.dowBgDval.top) {
						...
						if (y > this.downData.y - this.dowBgDval.top - this.tdSize) {
							...
							h =
								(Math.ceil((ylAbs - this.dowBgDval.top) / this.tdSize) + 1) *
									this.tdSize +
								"px";
						}
				...
				if (xl < 0) {
					if (x < this.downData.x - this.dowBgDval.left) {
						...
						if (x > this.downData.x - this.dowBgDval.left - this.tdSize) {
							...
							w =
								(Math.ceil((xlAbs - this.dowBgDval.left) / this.tdSize) + 1) *
									this.tdSize +
								"px";
						}
		...
		mousefn() {
			...
			var dayEnd =
				parseInt(this.moveBgData.height) / this.tdSize + dayStart - 1; //天数组索引 end
			var timeStart = parseInt(this.moveBgData.left) / this.tdSize; //小时数组（子）索引 start
			var timeEnd =
				parseInt(this.moveBgData.width) / this.tdSize + timeStart - 1; //小时数组（子）索引 end
...
.time_bloack_box > div > i {
	...
	text-align: center;
	color: #fff;
	background: $color_tiem_txt_bg;
}

.time_bloack_box > div > i:hover {
	font-size: 0;
	background: 0;
	height: 0;
}
