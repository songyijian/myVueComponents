      ...
      >
        ...
        >
          <span
            ...
            :key="'c' + index + indexf"
            :class="{ isselect: item == 1 }"
          >
            {{ index }}
          </span>
          <i
            v-for="(item, index) in infoStatue[indexf]"
            :style="{
              height: `${size}px`,
              'line-height': `${size}px`,
...
<script>
import { getRepeat, inState, undoState, copy } from "./tools.js";
...
export default {
  ...
  methods: {
        ...
        if (yl > 0) {
          if (y > this.downData.y + this.dowBgDval.bottom) {
            ...
            if (y < this.downData.y + this.dowBgDval.bottom + this.size) {
              ...
              h =
                (Math.ceil((yl - this.dowBgDval.bottom) / this.size) + 1) *
                  this.size +
                "px";
            }
          }
        ...
        if (xl > 0) {
          if (x > this.downData.x + this.dowBgDval.right) {
            ...
            if (x < this.downData.x + this.dowBgDval.right + this.size) {
              ...
              w =
                (Math.ceil((xl - this.dowBgDval.right) / this.size) + 1) *
                  this.size +
                "px";
            }
          }
        ...
        if (yl < 0) {
          if (y < this.downData.y - this.dowBgDval.top) {
            ...
            if (y > this.downData.y - this.dowBgDval.top - this.size) {
              ...
              h =
                (Math.ceil((ylAbs - this.dowBgDval.top) / this.size) + 1) *
                  this.size +
                "px";
            }
        ...
        if (xl < 0) {
          if (x < this.downData.x - this.dowBgDval.left) {
            ...
            if (x > this.downData.x - this.dowBgDval.left - this.size) {
              ...
              w =
                (Math.ceil((xlAbs - this.dowBgDval.left) / this.size) + 1) *
                  this.size +
                "px";
            }
    ...
    mousefn() {
      ...
      const dayEnd =
        parseInt(this.boxSelectionStyle.height) / this.size + dayStart - 1; //天数组索引 end
      const timeStart = parseInt(this.boxSelectionStyle.left) / this.size; //小时数组（子）索引 start
      const timeEnd =
        parseInt(this.boxSelectionStyle.width) / this.size + timeStart - 1; //小时数组（子）索引 end
